...
list <- data.frame(name = c("Top Volley Latina", "Cucine Lube Civitanova",
							"Revivre Milano", "Gi Group Monza", "Azimut Modena",
				   ...
				   code = c("LT", "MC", "MI-POWER", "MIVER", "MO", "BAM", "PD",
							"BASTIA", "PC", "RAV-ROB", "FR-SORA", "TN-ITAS",
							"VRI", "VV"))
...
for (i in 1:26){
	...
	a[[i]] <- y[[5]][, c(2, 4:5)]
	# Rename tables
	names(a[[i]])[2:3] <- paste0(c("Set", "Totali"), "_", gir[i], "_", rep(1:13, 2)[i])
}

## Combine list into df
x <- a[[1]]
for (i in 2:26){
	x <- left_join(x, a[[i]], by = "Club")
}

## Reorder
x <- x[, c(1, seq(2, 52, 2), seq(3, 53, 2))]
...
colnames(m) <- c(paste0("DiffSet_andata_", 1:13), paste0("DiffSet_ritorno_", 1:13),
				 paste0("DiffTot_andata_", 1:13), paste0("DiffTot_ritorno_", 1:13))

# Combine datasets
xx <- tbl_df(cbind(x, m))

# Save stat per team
save(xx, file = "../data/stat_per_team_tot.rda")

###############################################################################
## STATISTICS per team ATTACCO
###############################################################################
a <- list()
...
## Download tabelle girone di andata
for (i in 1:26){
	y <- read_html(paste0("http://www.legavolley.it/Rendimento.asp?Tipo=1&Classifica=1.3&AnnoInizio=2016&AnnoFine=2016&Serie=1&Fase=1&Giornata=", sq[i]), encoding = "ISO-8859-1") %>% html_table(fill = TRUE)
	# Extract the data table
	a[[i]] <- y[[5]][, c(2, 4:5)]
	# Rename tables
	names(a[[i]])[2:3] <- paste0(c("Set", "Att"), "_", gir[i], "_", rep(1:13, 2)[i])
}
...
# Rename columns
colnames(m) <- c(paste0("DiffSet_andata_", 1:13), paste0("DiffSet_ritorno_", 1:13),
				 paste0("DiffAtt_andata_", 1:13), paste0("DiffAtt_ritorno_", 1:13))

# Combine datasets
xx <- tbl_df(cbind(x, m))

# Save stat per team
save(xx, file = "../data/stat_per_team_att.rda")

###############################################################################
## STATISTICS per team BATTUTA
###############################################################################
a <- list()
...
## Download tabelle girone di andata
for (i in 1:26){
	y <- read_html(paste0("http://www.legavolley.it/Rendimento.asp?Tipo=1&Classifica=1.5&AnnoInizio=2016&AnnoFine=2016&Serie=1&Fase=1&Giornata=", sq[i]), encoding = "ISO-8859-1") %>% html_table(fill = TRUE)
	# Extract the data table
	a[[i]] <- y[[5]][, c(2, 4:5)]
	# Rename tables
	names(a[[i]])[2:3] <- paste0(c("Set", "ACE"), "_", gir[i], "_", rep(1:13, 2)[i])
}
...
# Save stat per team
save(xx, file = "../data/stat_per_team_ace.rda")

###############################################################################
## STATISTICS per team RICEZIONE
###############################################################################
a <- list()
...
## Download tabelle girone di andata
for (i in 1:26){
	y <- read_html(paste0("http://www.legavolley.it/Rendimento.asp?Tipo=1&Classifica=1.7&AnnoInizio=2016&AnnoFine=2016&Serie=1&Fase=1&Giornata=", sq[i]), encoding = "ISO-8859-1") %>% html_table(fill = TRUE)
	# Extract the data table
	a[[i]] <- y[[5]][, c(2, 4:5)]
	# Rename tables
	names(a[[i]])[2:3] <- paste0(c("Set", "Ric"), "_", gir[i], "_", rep(1:13, 2)[i])
}
...
# Save stat per team
save(xx, file = "../data/stat_per_team_ricez.rda")

###############################################################################
## STATISTICS per team MURO
###############################################################################
a <- list()
...
## Download tabelle girone di andata
for (i in 1:26){
	y <- read_html(paste0("http://www.legavolley.it/Rendimento.asp?Tipo=1&Classifica=1.9&AnnoInizio=2016&AnnoFine=2016&Serie=1&Fase=1&Giornata=", sq[i]), encoding = "ISO-8859-1") %>% html_table(fill = TRUE)
	# Extract the data table
	a[[i]] <- y[[5]][, c(2, 4:5)]
	# Rename tables
	names(a[[i]])[2:3] <- paste0(c("Set", "Muri"), "_", gir[i], "_", rep(1:13, 2)[i])
}
...
# Save stat per team
save(xx, file = "../data/stat_per_team_muro.rda")
...
for (i in 1:26){
	...
	a[[i]] <- y[[5]][, c(2, 4, 6:7)]
	# Rename tables
	names(a[[i]])[3:4] <- paste0(c("Set", "Totali"), "_", gir[i], "_", rep(1:13, 2)[i])
}
...
for (i in 1:26){
	...
	a[[i]] <- y[[5]][, c(2, 4, 6:7)]
	# Rename tables
	names(a[[i]])[3:4] <- paste0(c("Set", "Attacco"), "_", gir[i], "_", rep(1:13, 2)[i])
}
